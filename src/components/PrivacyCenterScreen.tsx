...
export const PrivacyCenterScreen: React.FC<PrivacyCenterScreenProps> = ({
  ...
  const filteredLogs = sensorLogs.filter((log) => {
    ...
    if (selectedSensorFilter === 'CAMERA') return log.sensorType === 'CAMERA';
    if (selectedSensorFilter === 'MIC') return log.sensorType === 'MIC';
    if (selectedSensorFilter === 'LOCATION') return log.sensorType === 'LOCATION';
    if (selectedSensorFilter === 'CLIPBOARD') return log.sensorType === 'CLIPBOARD';
    return true;
  });
        ...
        <div className="space-y-3 mb-6">
          ...
          <SecureDroidCard isLight={isLight}>
            <div className="flex items-center justify-between">
              ...
              <SecureDroidSwitch
                checked={privacyState.clipboardReadAlerts}
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${
                    ...
                    {log.sensorType === 'CAMERA' && <Camera className="w-4 h-4" />}
                    {log.sensorType === 'MIC' && <Mic className="w-4 h-4" />}
                    {log.sensorType === 'LOCATION' && <MapPin className="w-4 h-4" />}
                    {log.sensorType === 'CLIPBOARD' && <Copy className="w-4 h-4" />}
                    {log.sensorType === 'SENSORS' && <Eye className="w-4 h-4" />}
                  </div>
                  <div>
                    ...
                    <p className={`text-[11px] mt-0.5 ${isLight ? 'text-zinc-500' : 'text-zinc-400'}`}>
                      {log.wasAllowed ? 'Accessed sensor' : 'Blocked by killswitch policy'}
                    </p>
                  </div>
                </div>
                ...
                <div className="text-right">
                  <span className={`text-[10px] px-2 py-0.5 rounded font-mono ${
                    ...
                    {log.wasAllowed ? 'ALLOWED' : 'BLOCKED'}
                  </span>
